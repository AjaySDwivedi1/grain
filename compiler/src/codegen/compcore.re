...
let runtime_heap_start_name = Ident.unique_name(runtime_heap_start);
let runtime_heap_next_ptr = Ident.create_persistent("runtimeHeapNextPtr");
let runtime_heap_next_ptr_name = Ident.unique_name(runtime_heap_next_ptr);
let metadata_ptr = Ident.create_persistent("metadataPtr");
let metadata_ptr_name = Ident.unique_name(metadata_ptr);
...
let malloc_name = Ident.unique_name(malloc_ident);
...
let panic_with_exception_name = Ident.unique_name(panic_with_exception_ident);
let panic_with_exception_closure_ident =
  Ident.create_persistent("GRAIN$EXPORT$panicWithException");
let panic_with_exception_closure_name =
  Ident.unique_name(panic_with_exception_closure_ident);

/* Equality checking */
let equal_mod = "GRAIN$MODULE$runtime/equal.gr";
let equal_ident = Ident.create_persistent("equal");
let equal_name = Ident.unique_name(equal_ident);
...
let compile_bind =
  ...
  | MGlobalBind(slot, wasm_ty) =>
    let slot = Printf.sprintf("%s_%d", slot, env.dep_id);
    let slot = resolve_global(~env, slot);
    let typ = wasm_type(wasm_ty);
    switch (action) {
...
let call_lambda =
  ...
  switch (known) {
  | Some(name) =>
    let name = Printf.sprintf("%s_%d", name, env.dep_id);
    let name = resolve_func(~env, name);
    let instr =
      if (tail) {
  ...
  let func_idx =
    ...
    | Some(idx) =>
      Expression.Binary.make(
        ...
        Expression.Global_get.make(
          wasm_mod,
          Printf.sprintf("%s_%d", global_offset, env.dep_id),
          Type.int32,
        ),
        Expression.Const.make(wasm_mod, wrap_int32(idx)),
      )
...
let compile_function =
  ...
  // let func_name = Ident.unique_name(id);
  let func_name = Printf.sprintf("%s_%d", func_name, env.dep_id);
  let body_env = {
    ...env,
    num_args: arity,
  ...
  let compile_export = (i, export) => {
    ...
    | WasmGlobalExport({ex_global_internal_name, ex_global_name}) =>
      ...
      Export.add_global_export(
        wasm_mod,
        Printf.sprintf("%s_%d", ex_global_internal_name, env.dep_id),
        ex_global_name,
      );
    | WasmFunctionExport({ex_function_internal_name, ex_function_name}) =>
      ...
      Export.add_function_export(
        wasm_mod,
        Printf.sprintf("%s_%d", ex_function_internal_name, env.dep_id),
        ex_function_name,
      )
    };
  };
...
let compile_tables =
  ...
  let name = Printf.sprintf("elem_%d", env.dep_id);
  let global_name =
    Printf.sprintf(
      "%s_%d",
      Ident.unique_name(global_function_table_offset),
      env.dep_id,
    );
  let function_table_elements =
    List.map(
      elem => {
        let name = Printf.sprintf("%s_%d", elem, env.dep_id);
...
let compile_globals = (wasm_mod, env, {globals}) => {
  ...
  List.iter(
    ((id, mut, ty, initial)) => {
      let name = Printf.sprintf("%s_%d", Ident.unique_name(id), env.dep_id);
      // what?
      // let name =
      //   switch (Hashtbl.find_opt(env.import_resolutions, name)) {
      //   | Some((_, Some(resolution))) => resolution
      //   | _ => name
      //   };
      ignore @@
      Global.add_global(
...
let compile_main = (wasm_mod, env, prog) => {
  ...
  List.iteri(
    (dep_id, prog) => {
      let env = {...env, dep_id};
      // let name = Printf.sprintf("%s_%d", grain_main, dep_id);
      ignore @@
      compile_function(
        ~name=grain_main,
        wasm_mod,
...
let compile_type_metadata = (wasm_mod, env, prog) => {
  let metadata_tbl_data =
    if (Config.elide_type_info^) {
      ...
      let cmis = List.map(prog => prog.Mashtree.signature, prog.programs);
      let mds = List.map(x => x.Cmi_format.cmi_type_metadata, cmis);
